#include <iostream>
#include <locale>
#include "C:\Уник\AISD\SE_Lab06S\SE_Lab06L\Dictionary.h"
#pragma comment(lib,"C:\\Уник\\AISD\\SE_Lab06S\\SE_Lab06L\\Dictionary.h" )
...
int main()
{
    ...
    defined(TEST_GETENTRY_05) + defined(TEST_GETENTRY_06) + \
    defined(TEST_UPDENTRY_07) + defined(TEST_UPDENTRY_08) + \
    defined(TEST_DICTIONARY) > 1

#error "Должен быть определен только один тестовый макрос!" 

#endif
    try {
#if defined(TEST_CREATE_01)
        Dictionary::Instance d1 = Dictionary::Create("Очень длинное имя словаря которое превышает лимит", 5);
#elif defined(TEST_CREATE_02) 
        Dictionary::Instance d1 = Dictionary::Create("Словарь", 10000);
#else 
        Dictionary::Instance d1 = Dictionary::Create("Словарь", 5);
#endif
...
#endif
        std::cout << "------Преподователи------" << std::endl;
        Print(d1);
